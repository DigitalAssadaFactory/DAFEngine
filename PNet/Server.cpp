...
namespace PNet {
	...
	bool Server::Init(IPEndpoint ip)
	{
		...
		{
			OutputDebugStringA("Socket successfully created!\n");
			if (listeningSocket.Listen(ip) == PResult::P_Success)
			{
				...
				OutputDebugStringA("Socket successfully listening.\n");
				return true;
			}
			else
			{
				OutputDebugStringA("Failed to listen socket.\n");
			}
			listeningSocket.Close();
		}
		else
		{
			OutputDebugStringA("Socket creation failed.\n");
		}
	...
	void Server::Frame()
	{
		...
		if (WSAPoll(use_fd.data(), use_fd.size(), 1) > 0)
		{
			...
			if (listeningSocketFD.revents & POLLRDNORM)
			{
				...
				{
					OutputDebugStringA("Failed to accept new connection.\n");
				}
			}
		}
	...
	{
		std::string out = newConnection.ToString() + " - New connection accepted.\n";
		OutputDebugStringA(out.c_str());
	}

	void Server::OnDisconnect(TCPConnection& lostConnection, std::string reason)
	{
		std::string out = "[" + reason + "] Connection lost: " + lostConnection.ToString() + ".\n";
		OutputDebugStringA(out.c_str());
	}
	...
	{
		std::string out = "Packet received with size: " + std::to_string(packet->buffer.size()) + "\n";
		OutputDebugStringA(out.c_str());
		return true;
	}
