...
namespace PNet {
	...
	bool Client::Connect(IPEndpoint ip)
	{
		...
		if (socket.Create() == PResult::P_Success)
		{
			...
			{
				OutputDebugStringA("Failed to connect.\n");
			}
			socket.Close();
		}
		else
	...
	bool Client::Recv(std::shared_ptr<Packet> packet)
	{
	...
	{
		std::string out = "Sent packet with size: " + std::to_string(packet->buffer.size()) + "\n";
		OutputDebugStringA(out.c_str());
		return true;
	}
	...
	{
		std::string out = "Disconnected. " + reason + "\n";
		OutputDebugStringA(out.c_str());
	}
