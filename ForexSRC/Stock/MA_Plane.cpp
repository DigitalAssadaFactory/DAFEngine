...
namespace Stock {
	...
	MA_Plane::MA_Plane(const std::vector<Candle>& bars, const int& from,
		const int& length, const int& depth, const int& periodStride)
	{
		if (from - length < 0) Logger::ThrowBox("MA_Plane:\nInvalid range.");

		m_data = DYNAMIC_ARRAY_02_INIT(float, 0, 0);

		for (size_t i = from - length; i <= from; ++i)
		{
			m_data.push_back(std::vector<float>(0));
			for (size_t j = 0; j < depth; ++j)
			{
				if ((from - length - 1 - j * periodStride) < 0) break;
				m_data[i - (from - length)].push_back( MovingAverage(bars, from, 1 + periodStride * j) );
			}
		}
	}

	void MA_Plane::LoadGraphics()
	{
